...
def inside_algorithm(forest: CFG, tsort: list, edge_weights: dict) -> dict:
    ...
    for node in tsort:
        ...
        for rule in forest._rules_by_lhs[node]:
            w = 1  # edge_weights[rule] # TODO: make a valid call to dictionary values
            # node -> X, Y
...
def outside_algorithm(forest: CFG, tsort:list, edge_weights: dict, inside: dict) -> dict:
    ...
    for i, node in enumerate(reversed(tsort)):
        ...
        for rule in forest._rules_by_rhs[node]:
            k = O[rule._lhs] * 1 #edge_weights[rule]  # notation like in pseudocode #TODO: make edge weights work
            temp_prod = np.exp(1)  # perform computations in log scale so we will sum instead of mult.
...
def is_terminal(node):
    """
    Tests if node is terminal. The test is made based on observations of the structure, some adjustments might be
    necessary in the future.
    """
    if hasattr(node, "_symbol") and hasattr(node._symbol, "_symbol") and isinstance(node._symbol._symbol, Terminal):
        return True
    return False